/****************************************************************************
** Meta object code from reading C++ file 'pdfopts.h'
**
** Created: Sun Feb 24 21:37:15 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../scribus/pdfopts.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pdfopts.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_PDFExportDialog[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      16,   27,   27,   27, 0x09,
      28,   27,   27,   27, 0x09,
      41,   27,   27,   27, 0x09,
      59,   27,   27,   27, 0x09,
      72,   27,   27,   27, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_PDFExportDialog[] = {
    "PDFExportDialog\0DoExport()\0\0ChangeFile()\0"
    "fileNameChanged()\0enableSave()\0"
    "disableSave()\0"
};

void PDFExportDialog::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        PDFExportDialog *_t = static_cast<PDFExportDialog *>(_o);
        switch (_id) {
        case 0: _t->DoExport(); break;
        case 1: _t->ChangeFile(); break;
        case 2: _t->fileNameChanged(); break;
        case 3: _t->enableSave(); break;
        case 4: _t->disableSave(); break;
        default: ;
        }
    }
    Q_UNUSED(_a);
}

const QMetaObjectExtraData PDFExportDialog::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject PDFExportDialog::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_PDFExportDialog,
      qt_meta_data_PDFExportDialog, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &PDFExportDialog::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *PDFExportDialog::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *PDFExportDialog::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_PDFExportDialog))
        return static_cast<void*>(const_cast< PDFExportDialog*>(this));
    return QDialog::qt_metacast(_clname);
}

int PDFExportDialog::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
