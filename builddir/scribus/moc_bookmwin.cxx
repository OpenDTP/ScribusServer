/****************************************************************************
** Meta object code from reading C++ file 'bookmwin.h'
**
** Created: Sun Feb 24 21:37:04 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../scribus/bookmwin.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'bookmwin.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_BookMView[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: signature, parameters, type, tag, flags
      10,   22,   22,   22, 0x05,
      23,   22,   22,   22, 0x05,
      48,   22,   22,   22, 0x05,

 // slots: signature, parameters, type, tag, flags
      58,   81,   22,   22, 0x0a,
      85,  107,   22,   22, 0x0a,
     116,  163,   22,   22, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_BookMView[] = {
    "BookMView\0MarkMoved()\0\0SelectElement(PageItem*)\0"
    "changed()\0AddPageItem(PageItem*)\0ite\0"
    "ChangeText(PageItem*)\0currItem\0"
    "setPageItem(QTreeWidgetItem*,QTreeWidgetItem*)\0"
    "current,previous\0"
};

void BookMView::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        BookMView *_t = static_cast<BookMView *>(_o);
        switch (_id) {
        case 0: _t->MarkMoved(); break;
        case 1: _t->SelectElement((*reinterpret_cast< PageItem*(*)>(_a[1]))); break;
        case 2: _t->changed(); break;
        case 3: _t->AddPageItem((*reinterpret_cast< PageItem*(*)>(_a[1]))); break;
        case 4: _t->ChangeText((*reinterpret_cast< PageItem*(*)>(_a[1]))); break;
        case 5: _t->setPageItem((*reinterpret_cast< QTreeWidgetItem*(*)>(_a[1])),(*reinterpret_cast< QTreeWidgetItem*(*)>(_a[2]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData BookMView::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject BookMView::staticMetaObject = {
    { &QTreeWidget::staticMetaObject, qt_meta_stringdata_BookMView,
      qt_meta_data_BookMView, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &BookMView::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *BookMView::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *BookMView::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_BookMView))
        return static_cast<void*>(const_cast< BookMView*>(this));
    return QTreeWidget::qt_metacast(_clname);
}

int BookMView::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QTreeWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    }
    return _id;
}

// SIGNAL 0
void BookMView::MarkMoved()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void BookMView::SelectElement(PageItem * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void BookMView::changed()
{
    QMetaObject::activate(this, &staticMetaObject, 2, 0);
}
QT_END_MOC_NAMESPACE
