/****************************************************************************
** Meta object code from reading C++ file 'scrpalettebase.h'
**
** Created: Sun Feb 24 21:37:19 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../scribus/scrpalettebase.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'scrpalettebase.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_ScrPaletteBase[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      15,   34,   34,   34, 0x05,

 // slots: signature, parameters, type, tag, flags
      35,   34,   34,   34, 0x0a,
      57,   34,   34,   34, 0x0a,
      71,   86,   96,   34, 0x0a,
     100,   34,   34,   34, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_ScrPaletteBase[] = {
    "ScrPaletteBase\0paletteShown(bool)\0\0"
    "setPaletteShown(bool)\0setFontSize()\0"
    "exec(QWidget*)\0newParent\0int\0reject()\0"
};

void ScrPaletteBase::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        ScrPaletteBase *_t = static_cast<ScrPaletteBase *>(_o);
        switch (_id) {
        case 0: _t->paletteShown((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 1: _t->setPaletteShown((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 2: _t->setFontSize(); break;
        case 3: { int _r = _t->exec((*reinterpret_cast< QWidget*(*)>(_a[1])));
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = _r; }  break;
        case 4: _t->reject(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData ScrPaletteBase::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject ScrPaletteBase::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_ScrPaletteBase,
      qt_meta_data_ScrPaletteBase, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &ScrPaletteBase::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *ScrPaletteBase::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *ScrPaletteBase::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_ScrPaletteBase))
        return static_cast<void*>(const_cast< ScrPaletteBase*>(this));
    return QDialog::qt_metacast(_clname);
}

int ScrPaletteBase::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void ScrPaletteBase::paletteShown(bool _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
